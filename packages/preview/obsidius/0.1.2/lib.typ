...
#let emptyblock((fontcolor, bgcolor, bordercolor), content) = {
  ...
  block(width: 100%, inset: (x: 1em, y: 0.75em), radius: 0.5em, breakable: false,
...
#let compare(title_left, title_right, body_left, body_right) = {
  ...
  table(columns: (1fr, 1fr), align(center, title_left), align(center, title_right), body_left, body_right)
}
...
#let notes(title, content) = {
  ...
  show quote.where(block: true): it => {
    ...
    callout(emoji.bubble.speech.r, title, it.body, twColors.gray)
  }
  ...
  show image: i => {
      block(clip: true, radius: 0.5em, i)
  }
...
#let note(content) = callout(emoji.bubble.speech.r, [Note], content, twColors.gray)

#let summary(content) = callout(emoji.page.pencil, [Summary], content, twColors.teal)

#let fact(content) = callout(emoji.book.blue, [Fact], content, twColors.sky)

// todo

// hint

#let solution(content) = callout(emoji.checkmark.box, [Solution], content, twColors.green)

#let idea(content) = callout(emoji.lightbulb, [Idea], content, twColors.yellow)

#let question(content) = callout(emoji.quest, [Question], content, twColors.orange)

#let warning(content) = callout(emoji.warning, [Warning], content, twColors.orange)

#let problem(content) = callout(emoji.explosion, [Problem], content, twColors.red)

#let example(content) = callout(emoji.pencil, [Example], content, twColors.purple)
